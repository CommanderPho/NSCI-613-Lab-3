function [outputName, finalOutputPic, axesOnlyImage, plotOutputParts] = flattenGifGraph(importGifPath, should_output_plot_parts, should_show_figure, should_save_to_disk)
%UNTITLED Flattens an animated Gif of a plot generated by Neurons in Action
%to a single figure.
%   Detailed explanation goes here
% should_output_plot_parts: if true, returns component subplots for each
% frame.

	[parentPath, baseName, ext] = fileparts(importGifPath);

	if ~exist('should_show_figure','var')
		should_show_figure = false;
	end
	
	if ~exist('should_save_to_disk','var')
		should_save_to_disk = false;
	end
		
% 	should_output_plot_parts = true;
	if ~should_output_plot_parts
		plotOutputParts = {};
	end
...
% 	outputPlotParts
	...
	for i=1:numberOfFrames
		...
		if should_output_plot_parts
			currPlotPartOutput.plotMask_BW = fixedImage_BW;
		end
	...
	% 		curr_color = color_map(i,:);
		...
		if should_output_plot_parts
			currPlotPartOutput.weighted_image = weighted_image;
		end
		...
		if should_output_plot_parts
			plotOutputParts{i} = currPlotPartOutput;
		end
	end
